...
if False:
  ...
  for kernel in Kernels:  # Loop over different kernels
    print(
        f'& {kernel} & '
        f'{np.round(np.mean(subsetGrid1296[subsetGrid1296["kernel"] == kernel]["mape"].to_numpy()), 4)} & '
        f'{np.round(np.std(subsetGrid1296[subsetGrid1296["kernel"] == kernel]["mape"].to_numpy()), 4)} & '
        f'{np.round(np.mean(subsetGrid1296[subsetGrid1296["kernel"] == kernel]["r2"].to_numpy()), 4)} & '
        f'{np.round(np.std(subsetGrid1296[subsetGrid1296["kernel"] == kernel]["r2"].to_numpy()), 4)} \\\\'
    )
  print('')
  print(f'Grid2401')
  for kernel in Kernels:  # Loop over different kernels
    print(
        f'& {kernel} & '
        f'{np.round(np.mean(subsetGrid2401[subsetGrid2401["kernel"] == kernel]["mape"].to_numpy()), 4)} & '
        f'{np.round(np.std(subsetGrid2401[subsetGrid2401["kernel"] == kernel]["mape"].to_numpy()), 4)} & '
        f'{np.round(np.mean(subsetGrid2401[subsetGrid2401["kernel"] == kernel]["r2"].to_numpy()), 4)} & '
        f'{np.round(np.std(subsetGrid2401[subsetGrid2401["kernel"] == kernel]["r2"].to_numpy()), 4)} \\\\'
    )
  print('')
  print(f'Sobol1')
  for kernel in Kernels:  # Loop over different kernels
    print(
        f'& {kernel} & '
        f'{np.round(np.mean(subsetSobol1[subsetSobol1["kernel"] == kernel]["mape"].to_numpy()), 4)} & '
        f'{np.round(np.std(subsetSobol1[subsetSobol1["kernel"] == kernel]["mape"].to_numpy()), 4)} & '
        f'{np.round(np.mean(subsetSobol1[subsetSobol1["kernel"] == kernel]["r2"].to_numpy()), 4)} & '
        f'{np.round(np.std(subsetSobol1[subsetSobol1["kernel"] == kernel]["r2"].to_numpy()), 4)} \\\\'
    )
  print('')
  print(f'Sobol2')
  for kernel in Kernels:  # Loop over different kernels
    print(
        f'& {kernel} & '
        f'{np.round(np.mean(subsetSobol2[subsetSobol2["kernel"] == kernel]["mape"].to_numpy()), 4)} & '
        f'{np.round(np.std(subsetSobol2[subsetSobol2["kernel"] == kernel]["mape"].to_numpy()), 4)} & '
        f'{np.round(np.mean(subsetSobol2[subsetSobol2["kernel"] == kernel]["r2"].to_numpy()), 4)} & '
        f'{np.round(np.std(subsetSobol2[subsetSobol2["kernel"] == kernel]["r2"].to_numpy()), 4)} \\\\'
    )
  print('')
  exit()
...
for nSubset in range(0, len(datasetSubsets)):
  ...
  axd['AvgByKernel'].errorbar(KernelsForPlotting, AvgRMSE, yerr=AvgRMSEerr, label=f'Average {xLabel}', marker='o', color='#069AF3', ls='-.')
  ...
  axd["AvgByKernel"].set_title(f'Avg. {xLabel}/{yLabel} per kernel', fontweight='bold')
  ...
  axd2.errorbar(KernelsForPlotting, AvgR2, yerr=AvgR2err, label=f'Average R2s', marker='o', color='#F97306', ls='-.')
