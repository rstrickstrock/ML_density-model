...
good_energy_file1 = 'prevOpt_energies_sorted.txt'
good_energy_file2 = 'thisOpt_energies_sorted.txt'

num_energies = np.linspace(1, 96, 96)
target_energies = read_energy_file(target_energy_file)
#print(f'{target_energies}')
good_energies1 = read_energy_file(good_energy_file1)
good_energies2 = read_energy_file(good_energy_file2)
...
plt.plot(num_energies, good_energies1, '-.o', color='#FF800E', label='RCE using the previously optimized parameter set', zorder=2, linewidth=line_width, ms=marker_size, mew=marker_edge_width)
plt.plot(num_energies, good_energies2, '-.d', color='#5D3A9B', label='RCE using the currently optimized parameter set', zorder=2, linewidth=line_width, ms=marker_size, mew=marker_edge_width)
