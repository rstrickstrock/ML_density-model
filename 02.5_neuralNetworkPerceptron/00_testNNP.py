...
statisticsFileName = f'Stats-{dataset}-{testSize[0]}.csv'
...
for thisRatio in testSize:
  ...
  for rndInt in rndInts:
    ...
    X_TRAIN1296 = torch.FloatTensor(X_train.to_numpy())
    Y_TRAIN1296 = torch.FloatTensor(Y_train.to_numpy())
    Train1296 = TensorDataset(X_TRAIN1296, Y_TRAIN1296)
    ...
    X_TEST1296 = torch.FloatTensor(X_test.to_numpy())
    Y_TEST1296 = torch.FloatTensor(Y_test.to_numpy())
    Test1296 = TensorDataset(X_TEST1296, Y_TEST1296)
    
    ## setting batch size and dataloader
    batchSize1296 = len(Train1296)
    
    train1296DL = DataLoader(Train1296, batchSize1296, shuffle=True)
    test1296DL = DataLoader(Test1296, batchSize1296, shuffle=True)
    ...
    for epoch in range(num_epochs):
      print(f'  epoch: {epoch}')
      for step, (xb, yb) in enumerate(train1296DL):
        #print(f'  xb: {xb}')
        #print(f'  yb: {yb}')
        ...
        if epoch == 0:
          ...
          thisR2 = 0
          i = 0
          for tmpStep, (tmpXB, tmpYB) in enumerate(test1296DL):
